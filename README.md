...
当前阶段不直接进入正式正文写作。P0 假设登记、P0.5 合规与相似风险边界、P1 L0 样本池已经完成初版；P2-alpha 三本前 10 章校准批次已完成。当前先插入 P2.1 单章 clean-room 复刻校准，验证小说版“提取 -> 归纳 -> 复刻 -> 盲审 -> 回证据修 packet”闭环，再扩到 P2-beta 的 10-15 本正式快拆。

## 当前主线

当前不继续补抽象方法论，先按总控路线做 P2.1 单章复刻校准：

1. 先确认番茄、七猫、起点的直发/内投/签约差异。
2. 研究各平台有哪些钱可以拿、触发条件和更新成本。
3. 把平台机制字段接入 P2 证据提取协议。
4. P2-alpha 已从 L0 样本池中选 3 本完成校准：源头/开山样本、官方榜单当前高热样本、官方榜单高武系统样本。
5. P2-alpha 后先跑一轮 P2.1 单章 clean-room 复刻校准。
6. P2.1 跑通后，再扩到 P2-beta 的 10-15 本前 10 章快拆。
7. 在样本证据和诊断闭环完成前，暂停正式正文、完整世界观、作者级 skill 和百万字大纲。
...
- [docs/20_小说蒸馏方法/07_小说单章clean-room复刻诊断协议.md](docs/20_小说蒸馏方法/07_小说单章clean-room复刻诊断协议.md): 对齐文案复刻闭环的小说单章提取、归纳、clean-room、三找一盲审和回修协议。
...
- [.agents/skills/novel-chapter-distiller/SKILL.md](.agents/skills/novel-chapter-distiller/SKILL.md): 单章 source-open、证据提取、diagnostic packet 和 clean-room 单章复刻。
- [.agents/skills/novel-blind-forensic-review/SKILL.md](.agents/skills/novel-blind-forensic-review/SKILL.md): 小说单章/开局三找一盲审、AI 水文和相似风险诊断。
...
4. 已完成 P0.6：三平台够用表和待验证清单分流。
5. 已完成 P2-alpha：3 本前 10 章校准批次。
6. 下一步进入 P2.1：单章 clean-room 复刻校准。
7. P2.1 跑通后，才进入 P2-beta：10-15 本前 10 章快拆。
8. 保持 clean-room 写作纪律：研究样本可以拆结构，诊断稿不能搬运表达，正式正文不能搬运表达。
