...
当前阶段不直接进入正文写作，先完成 P0/P1：假设登记和 L0 样本池。目标是先用证据判断方向是否值得立项，而不是靠设定兴趣或单篇 prompt 推进。

## 当前主线

当前不继续补方法论，先按总控路线做 P0/P1：

1. 登记平台、题材、改编、AI 风险假设。
2. 建立番茄都市高武/异常事件方向 L0 样本池。
3. 用样本证据决定是否进入前 10 章快拆。
...
1. 完成 P0 假设登记。
2. 完成 P1 L0 样本池：20-30 本番茄同赛道样本。
3. 只在样本来源、标签、简介、采样日期清楚后进入 L1。
